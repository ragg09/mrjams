    ...
    function clinic_data() {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data.status == 0 ){
                    $("#info").empty();
                    $("#info").append('<div style="width:60%; margin-left: 20%; margin-right: 20%; padding-top:20px;"><h5 class="card-header" style="background-color:#B3CDE0; font-size: 15px;"></h5><div class="card-body" style="text-align:center; width:100%; border-bottom: 5px solid #B3CDE0;"><h5 class="card-title" style="font-size: 45px;">No More Data</h5></div></div>');

                }else{
                    $("#info").empty();
                    $.each(data.all, function(index, val){
                        $("#info").append('<div style="width:60%; margin-left: 20%; margin-right: 20%; padding-top:20px;"><h5 class="card-header" style="background-color:#B3CDE0; font-size: 15px; font-weight:bold;">'+val.type_of_clinic+' Clinic | Address: '+val.address_line_1+', '+val.address_line_2+'</h5><div class="card-body" style="text-align:center; width:100%; border-bottom: 5px solid #B3CDE0;"><h5 class="card-title" style="font-size: 45px;">'+val.name+'</h5><p class="card-text"><strong>Packages: </strong>'+val.package+'... | <strong>Services: </strong>'+val.service+'...</p><br></br><a href="/customer/appointment/'+val.id+'" class="btn btn-primary" style="background-color:#6497B1; magin-top: 5px;">View More</a></div></div>');
                   });
