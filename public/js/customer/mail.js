                ...
                if(data.status == 0 ){
                    ...
                    $.each(data.all, function(index, val){
                        ...
                        if(val.status == 5){
                            ...
                            $("#info").append('<tr><th class="text-center"><i class="fa fa-star text-'+color+'"></i><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;"><b>'+val.name+'</b></a></td><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;">Your Appointment has been Declined</b>....</a></td><td class="text-center"><button class="btn btn-'+color+'" id="stat" style="border-radius: 8px;">'+val.remark+'</button></td><td class="text-center">'+datesToYMD(new Date(create_appoint))+'</td> <td class="text-center"><a href="#" data-bs-toggle="modal" data-bs-target="#delete_modal_up" id="delete_modal" data-id="'+val.id+'" ><i class="fa fa-trash text-dark" aria-hidden="true"></i></a></td></tr>');
                            ...
                            var date_time = year + '-' + month + '-' + day + ' ' + (today.getHours() + 1) + ':00';
                            if(date_time === date_appoint + " " +time_appoint){
                                console.log("bawal na icancel");
                                $("#info").append('<tr><th class="text-center"><i class="fa fa-star text-'+color+'"></i><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;"><b>'+val.name+'</b></a></td><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;">Your appointment is scheduled on <b>'+dateToYMD(new Date(date_appoint))+'</b> at <b>'+tConv24(time_appoint)+'</b>...</a></td><td class="text-center"><button class="btn btn-'+color+'" id="stat" style="border-radius: 8px;">'+val.remark+'</button></td><td class="text-center">'+datesToYMD(new Date(create_appoint))+'</td><td class="text-center"><a href="#" data-bs-toggle="modal" data-bs-target="#cancel_modal_up" id="cancel_modal" data-id="'+val.id+'" style="pointer-events: none;"><i class="fa fa-ban text-secondary" aria-hidden="true"></i></a></tr>');
                            }else{
                                console.log("pwede pa icancel");
                                $("#info").append('<tr><th class="text-center"><i class="fa fa-star text-'+color+'"></i><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;"><b>'+val.name+'</b></a></td><td><a href="/customer/mail/'+val.id+'" style="color:black; text-decoration: none;">Your appointment is scheduled on <b>'+dateToYMD(new Date(date_appoint))+'</b> at <b>'+tConv24(time_appoint)+'</b>...</a></td><td class="text-center"><button class="btn btn-'+color+'" id="stat" style="border-radius: 8px;">'+val.remark+'</button></td><td class="text-center">'+datesToYMD(new Date(create_appoint))+'</td><td class="text-center"><a href="#" data-bs-toggle="modal" data-bs-target="#cancel_modal_up" id="cancel_modal" data-id="'+val.id+'" ><i class="fa fa-ban text-danger" title="Please keep in mind that you cannot cancel an appointment less than one hour before to the scheduled time; If you want the transaction to proceed smoothly, it is best to contact the clinic." aria-hidden="true"></i></a></tr>');
                            }
